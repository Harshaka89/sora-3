...
class YRR_Coupons_Model {
    ...
    public static function validate_coupon($code) {
        ...
        if (!empty($coupon->valid_to) && strtotime($coupon->valid_to) < current_time('timestamp')) {
            return new WP_Error('expired_coupon', __('This coupon has expired.', 'yrr'));
        }
        if ($coupon->usage_limit !== null && $coupon->usage_count >= $coupon->usage_limit) {
            return new WP_Error('limit_exceeded', __('This coupon has reached its usage limit.', 'yrr'));
        }
    ...
    public static function increment_usage($id) {
        global $wpdb;
        $wpdb->query($wpdb->prepare("UPDATE " . YRR_COUPONS_TABLE . " SET usage_count = usage_count + 1 WHERE id = %d", $id));
    }
}
