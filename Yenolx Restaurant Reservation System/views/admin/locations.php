...
<div class="wrap">
  <div style="max-width:1180px;margin:22px auto 25px auto;background:white;padding:13px 0 16px 0;border-radius:12px;box-shadow:0 6px 18px rgba(0,0,0,0.09);">
    <div style="text-align:center;margin-bottom:6px;padding-bottom:10px;border-bottom:2px solid #ffc107;">
      <h1 style="font-size:1.5rem;color:#2c3e50;padding-bottom:2px;margin:0;">🏢 Locations & Dining Areas</h1>
      <p style="color:#6c757d;margin:2px 0 0;font-size:0.95rem;">Manage all dining zones, rooms, patios, or bar areas. Link these to each table.</p>
    </div>
    <!-- Add Location Form -->
    <div style="background:#fffbea;padding:13px 18px 7px 18px;border-radius:10px;margin:10px 22px 18px 22px;border:2px solid #ffc107;">
      <form method="post">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;">
          ...
          <input type="text" name="notes" maxlength="32" placeholder="Notes (e.g., Smoking, VIP)" style="padding:8px 7px;border-radius:7px;border:1.5px solid #e9ecef;">
        </div>
        <div style="text-align:right;margin-top:7px;">
          <button type="submit" class="button button-primary" style="padding:7px 23px;font-size:1.03em;border-radius:6px;">➕ Add Location</button>
        </div>
      </form>
    </div>
    <!-- Location Card Grid 8 per row -->
    <div style="display:grid;grid-template-columns:repeat(8,minmax(104px,1fr));gap:8px;">
      ...
      <div style="background:#fff;border:2px solid #ffc107;border-radius:13px;padding:11px 3px 10px 6px;position:relative;min-height:88px;box-sizing:border-box;">
        <div style="text-align:center;margin-bottom:2px;">
          <span style="display:inline-block;font-size:1.19rem;font-weight:900;color:#e8a800;margin-bottom:0px;"><?php echo esc_html($loc->name); ?></span>
        ...
        <div style="font-size:0.95em;text-align:center;color:#363e52;margin-bottom:2px;word-break:break-all;"><?php echo esc_html($loc->notes); ?></div>
        <div style="margin-top:3px;text-align:center;">
          <button onclick="editLoc(<?php echo htmlspecialchars(json_encode($loc)); ?>)" type="button"
            style="background:#ffc107;color:#333;border:none;padding:3px 8px;border-radius:7px;font-size:0.95em;font-weight:700;cursor:pointer;">✏️ Edit</button>
          <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=yrr-locations&delete_location='.$loc->id), 'yrr_location_action'); ?>"
             ...
             style="background:#dc3545;color:white;padding:3px 8px;text-decoration:none;border-radius:7px;font-size:0.95em;font-weight:700;">
            🗑️ Delete
          </a>
        </div>
      </div>
...
<div id="editLocModal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:10000;align-items:center;justify-content:center;">
  <div style="background:white;padding:18px;border-radius:13px;width:95%;max-width:320px;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:9px;">
      <h3 style="margin:0;font-size:1.15em;">✏️ Edit Location</h3>
